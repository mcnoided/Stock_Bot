...
num = 150
stocks = []
size = len(tickers[:num])
for i in range(size):
    print(f'{i+1}/{size}: ')
    stock = Stock(tickers[i+160])
...
def qualifies(i, j):
    if np.abs(z_matrix[i][j]) > 2 and corr_matrix[i][j] > 0.7 and i != j:
        return True
    else:
        return False

returns = np.zeros((size,1))

for i in range(size):
    percentages = []
    for j in range(size):
        change = z_info_matrix[i][j][1]
        if qualifies(i, j):
            percentages.append(change)
    if len(percentages) == 0:
        percentages.append(0)
    #print(percentages)
    returns[i] = np.mean(percentages)

returns = (np.random.rand(size,1)*2-1)/100
#print(returns)
...
for start in range(step):
    print(start)
    cov_segment = cov_matrix[start::step,start::step]
    returns_segment = returns[start::step]
    segment_size = returns_segment.size
    print(segment_size)
    print(np.shape(cov_segment))
    A = np.block([[2*cov_segment,-returns_segment,-np.ones((segment_size,1))],
                  [returns_segment.transpose(),0,0],
                  [np.ones((1,segment_size)),0,0]])
    segment_portfolio = np.zeros(size)
    for mu in np.linspace(np.min(returns), np.max(returns), 100):
        b = np.zeros(segment_size+2)
        b[segment_size] = mu
        b[segment_size+1] = 1
        x = np.linalg.solve(A, b)
        omega = x[:segment_size]
        risk = omega.transpose() @ cov_segment @ omega
        k = (mu-risk_free)/risk
        if k > largest_k:
            largest_k = k
            segment_portfolio = omega
    shuffled.extend(segment_portfolio)
unshuffled = []
for start in range(step):
    unshuffled.extend(shuffled[start::step])
print(unshuffled)

A = np.block([[2*cov_matrix,-returns,-np.ones((size,1))],
              [returns.transpose(),0,0],
              [np.ones((1,size)),0,0]])
